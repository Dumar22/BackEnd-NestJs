...
import { CreateDetailExitMaterialsDto, CreateExtiMaterialsAditionalsServiceDto, UpdateDetailExitMaterialsDto, UpdateExtiMaterialsAditionalsServiceDto } from './dto';
...
export class ExtiMaterialsAditionalsServicesService {
  ...
  constructor(
    ...
    private readonly contractRepository: Repository<ContractPostService>,
    @InjectRepository(ExtiMaterialsAditionalsService)
    private readonly exitMaterialsRepository: Repository<ExtiMaterialsAditionalsService>,
    @InjectRepository(ExtiMaterialsAditionalsDetailsService)
    private readonly detailsExitRepository: Repository<ExtiMaterialsAditionalsDetailsService>,
  ) {  pdfMake.vfs = pdfFonts.pdfMake.vfs;}

  async create(
    createexitMaterialsDto: CreateExtiMaterialsAditionalsServiceDto, details: CreateDetailExitMaterialsDto[], user: User,) {
...
private async getLastExitNumberForUser(warehouseId: string): Promise<number> {
  ...
  async update(id: string , updateExitMaterialsDto: UpdateExtiMaterialsAditionalsServiceDto, details: UpdateDetailExitMaterialsDto[], newDetails: CreateDetailExitMaterialsDto[] | undefined, user: User, ) {
